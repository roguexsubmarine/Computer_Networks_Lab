...
def calculate_network_broadcast(ip, subnet_mask):
    ...
    number_of_networks = 2**(int(subnet_mask) - 8)
...
def main():
    ...
    subnet_mask = int(input("Enter the subnet mask (8 - 30): /"))
    while subnet_mask < 8 or subnet_mask > 30:
        print("Enter valid subnet mask !")
        subnet_mask = int(input("Enter the subnet mask (8 to 30): /"))


    ip_temp = ['0', '0', '0', '0']
    while int(ip_temp[0]) < 1 or int(ip_temp[0]) > 127:
        ip_address = input("Enter the valid IP address : ")
