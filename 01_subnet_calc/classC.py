...
def calculate_network_broadcast(ip, subnet_mask):
    ...
    number_of_networks = 2**(int(subnet_mask) - 24)
...
def main():
    ...
    subnet_mask = int(input("Enter the subnet mask (in format /27): /"))
    while subnet_mask < 24 or subnet_mask > 30:
        print("Enter valid subnet mask !")
        subnet_mask = int(input("Enter the subnet mask (24 to 30): /"))

    ip_address = int(input("Enter the IP address : 192.168.1."))
    while ip_address < 1 or ip_address > 255:
        print("Enter valid last octet !")
        ip_address = int(input("Enter the IP address : 192.168.1."))
    
    print("")

    ip_address = "192.168.1."+str(ip_address)
